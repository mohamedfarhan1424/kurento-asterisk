...
const OVERLAY_URL =
  "https://github.com/Kurento/kurento-tutorial-node/raw/master/kurento-magic-mirror/static/img/mario-wings.png";
...
KurentoMediaHandler.prototype = {
  ...
  getDescription: function (onSuccess, onFailure, mediaHint) {
    ...
    if (user.asteriskSdp.type === "answer") {
      ...
      user.pipeline.rtpEndPoint
        .processOffer(user.asteriskSdp.value)
        .then(onSuccess)
        .catch(onFailure);
    }
  },
      ...
      pipeline.create(
        ...
        function (error, [rtpEndPoint, webEndPoint, faceFilter]) {
          ...
          webEndPoint.on("IceCandidateFound", function (event) {
            ...
            userRegistry.getById(userId).ws.send(
              JSON.stringify({
                id: "iceCandidate",
                candidate: candidate,
              })
  ...
  if (callResponse === "accept") {
    createCallPipeline(callee, (error, sdpAnswer) => {
      /* ***************************************************
            ...
            *************************************************** */
      callee.pipeline.rtpEndPoint
        .processOffer(callee.asteriskSdp.value)
        .then((answer) => {
          callee.asteriskSdp = {
            type: "answer",
            value: answer,
